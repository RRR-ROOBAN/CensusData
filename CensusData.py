...
Client=pymongo.MongoClient("mongodb+srv://Roobanruban:Roobanatlas@cluster0.6saiv.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0") #server

newdb=Client['GUVI_Project']  #Database

Collection=newdb["Census"]

data_to_upload=df.to_dict(orient="records") #converting dataframe to dictionary

Collection.insert_many(data_to_upload) #upload covert data to database
...
mydb =mysql.connector.connect(
  ...
  port = 4000,
  user = "3u3gGZU682g7SQc.root",
  password = "NZrDZvZZBRcy9LQ9",
  database = "Guvi_Sql",
...
engine=sqlalchemy.create_engine("mysql+mysqlconnector://3u3gGZU682g7SQc.root:NZrDZvZZBRcy9LQ9@gateway01.ap-southeast-1.prod.aws.tidbcloud.com:4000/Guvi_Sql")
...
def fetch_data(query):
    ...
    df3 = pd.read_sql(query, con=connection)  #Data Base to sql
    connection.close()
    return df3
